...
=head1 Usage
  ...
  --queue <str>     specify the queue to use, default st.q
  --ProjectID <str> specify the ProjectID to use, default st_singlecell
...
my ($Queue, $ProjectID, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,$Job_prefix,$Verbose, $Help);
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"interval:i"=>\$Interval,
...
while (@Shell) {
	...
	$job_cmd .= "-P $ProjectID "  if(defined $ProjectID); ##set queue#################################have changed at 2014/5/23---14:55
	$job_cmd .= "-q $Queue "  if(defined $Queue); ##set queue#################################have changed at 2013/3/11---10:01
